...
/**
 * {@link DcpChannel#closeStream} is either handed to the channel and recorded, or neither (MB-73569). A caller which
 * sees it throw may take it that nothing was sent; that holds only if nothing was recorded either, so the stream must
 * not be marked {@code DISCONNECTING} or dropped from the channel's open streams unless the close request was handed
...
public class DcpChannelCloseStreamTest {
    ...
    /**
     * A channel which accepts the write and fails it afterwards, the way a connection the producer has dropped does:
     * the caller sees a normal return, and only the future knows.
     */
    private static final class FailingChannel extends EmbeddedChannel {
        @Override
        public ChannelFuture writeAndFlush(Object msg) {
            ReferenceCountUtil.release(msg);
            return newFailedFuture(new IOException("simulated connection reset"));
        }
    }
    ...
    @Test
    public void testFailedFutureSettlesTheClose() {
        SessionState sessionState = sessionState();
        DcpChannel dcpChannel = connectedChannel(sessionState, new FailingChannel());
        StreamPartitionState partition = sessionState.streamState(SID).get(VB);

        dcpChannel.closeStream(SID, VB);

        // the close was handed over, so it is recorded; a request which could not be sent will never be answered, so
        // the failure listener settles it rather than leaving it DISCONNECTING for good- and it must not have been
        // overtaken by the recording, which would leave exactly that
        Assert.assertEquals("partition state", StreamPartitionState.DISCONNECTED, partition.getState());
        Assert.assertFalse("no longer an open stream", dcpChannel.openStreams()[VB].contains(SID));
    }
