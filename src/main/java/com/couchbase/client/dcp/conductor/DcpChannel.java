    ...
    private void writeCloseStream(final int streamId, final short vbid) {
        ...
        channel.writeAndFlush(buffer).addListener(f -> {
            if (!f.isSuccess()) {
                ...
                // settled as closed here- as the response handler does for a close which the producer rejects. Under
                // the channel's monitor, as the response handler is: our caller holds it until the close below has
                // been recorded, so a write which fails before then cannot be settled first and then overwritten
                synchronized (DcpChannel.this) {
        ...
        // recorded only once the close has been handed to the channel, so that a write path which throws leaves the
        // stream as it was: still open as far as we know, and still streamed by the producer (MB-73569)
